...
(define church-list->lazy-list
  (lambda (address store . args)
    (apply list->lazy-list args)))
(define church-lazy-equal?
  (lambda (address store . args) (apply lazy-equal? args)))
(define church-lazy-list->list
  (lambda (address store . args)
    (apply lazy-list->list args)))
(define church-<
  (lambda (address store . args) (apply < args)))
...
(define church-curry
  (lambda (address store . args) (apply curry args)))
(define church-sym
  (lambda (address store . args) (apply sym args)))
(define church-remove
  (lambda (address store . args) (apply remove args)))
...
(define church-pair
  (lambda
    (address store #{g0 |9efxRRNhG3v9Lsfp|}
      #{g1 |8P=un$!CFDaK8/0w|})
    (cons #{g0 |9efxRRNhG3v9Lsfp|} #{g1 |8P=un$!CFDaK8/0w|})))
(define church-first
  (lambda (address store #{g2 |kPFjTQDt1!PjFh6H|})
    (car #{g2 |kPFjTQDt1!PjFh6H|})))
(define church-rest
  (lambda (address store #{g3 |5rHXsn$2wV0RFmQ/|})
    (cdr #{g3 |5rHXsn$2wV0RFmQ/|})))
           ...
           (church-gen-program
            (lambda (address store)
              ((lambda (address store church-num-of-funcs)
                    ...
                    ((lambda
                       ...
                       ((lambda (address store church-body)
                          (church-combine-program-parts
                            (cons 'a696 address) store
                            church-functions church-body))
                         (cons 'a697 address) store
                         (church-gen-expr
                           (cons 'a698 address) store '()
                           church-function-signatures)))
                      (cons 'a699 address) store
                      (church-map (cons 'a700 address) store
                        (church-curry (cons 'a701 address)
                          store church-gen-function
                          church-function-signatures)
                        church-function-signatures)))
                ...
                (church-random-integer (cons 'a705 address)
                  store church-MAX-FUNCS))))
           (church-combine-program-parts
            (lambda
              (address store church-functions church-body)
              (church-append (cons 'a706 address) store
                (church-append (cons 'a707 address) store
                  (church-list (cons 'a708 address) store
                    'let '())
                  church-functions)
                (church-list (cons 'a709 address) store
                  church-body))))
           (church-get-name church-first)
           (church-get-vars church-rest)
           (church-get-arity
            (lambda (address store church-signature)
              (church-length (cons 'a710 address) store
                (church-get-vars (cons 'a711 address) store
                  church-signature))))
           (church-gen-func-signatures
            (lambda (address store church-num-of-funcs)
              (church-repeat (cons 'a712 address) store
                church-num-of-funcs
              ...
              ((lambda (address store church-name)
                 ((lambda (address store church-arity)
                    ((lambda (address store church-vars)
                      ...
                      (church-gen-vars (cons 'a715 address)
                        store church-arity)))
                ...
                (church-sym (cons 'a719 address) store 'F))))
           (church-combine-sig-parts church-pair)
           (church-gen-vars
            (lambda (address store church-arity)
              (church-repeat (cons 'a720 address) store
                church-arity
                (lambda (address store)
                  (church-sym (cons 'a721 address) store 'V)))))
           (church-gen-function
            (lambda
                 ...
                 ((lambda (address store church-vars)
                    ((lambda (address store church-body)
                       (church-combine-function-parts
                         ...
                         church-body))
                      (cons 'a723 address) store
                      (church-gen-expr (cons 'a724 address)
                        store church-vars
                        church-function-signatures)))
                   (cons 'a725 address) store
                   (church-get-vars (cons 'a726 address)
                     store church-function-signature)))
                (cons 'a727 address) store
                (church-remove (cons 'a728 address) store
                  church-function-signature
                  church-function-signatures))))
           (church-combine-function-parts
            (lambda
              (address store church-function-signature
                church-body)
              (church-list (cons 'a729 address) store
           ...
           (church-gen-expr
            (lambda
              (address store church-vars
                church-function-signatures)
              (if (church-null? (cons 'a730 address) store
                    church-vars)
                  (church-multinomial (cons 'a731 address)
                    store
                    (church-list (cons 'a732 address) store
                      (church-gen-if (cons 'a733 address)
                        store church-vars
                        church-function-signatures)
                      (church-gen-bool (cons 'a734 address)
                        store)
                      (church-gen-application
                        (cons 'a735 address) store
                        church-vars
                        church-function-signatures))
                    (church-list (cons 'a736 address) store
                      1/8 3/4 1/8))
                  (church-multinomial (cons 'a737 address)
                    store
                    (church-list (cons 'a738 address) store
                      (church-gen-if (cons 'a739 address)
                        store church-vars
                        church-function-signatures)
                      (church-gen-bool (cons 'a740 address)
                        store)
                      (church-gen-application
                        (cons 'a741 address) store
                        church-vars
                        church-function-signatures)
                      (church-gen-var (cons 'a742 address)
                        store church-vars))
                    (church-list (cons 'a743 address) store
                      1/12 3/4 1/12 1/12)))))
           (church-gen-bool
            (lambda (address store)
              (church-uniform-draw (cons 'a744 address)
                store '(t f))))
           (church-gen-if
            (lambda
              (address store church-vars
                church-function-signatures)
              (church-combine-if-parts (cons 'a745 address)
                store 'if
                (church-gen-expr (cons 'a746 address) store
                  church-vars church-function-signatures)
                (church-gen-expr (cons 'a747 address) store
                  church-vars church-function-signatures)
                (church-gen-expr (cons 'a748 address) store
                  church-vars church-function-signatures))))
           (church-combine-if-parts church-list)
           (church-gen-application
            (lambda
              (address store church-vars
                church-function-signatures)
                 ...
                 ((lambda (address store church-function)
                    ((lambda (address store church-arity)
                       ((lambda (address store church-args)
                         ...
                         (church-gen-args
                           (cons 'a751 address) store
                           church-arity church-vars
                           church-function-signatures)))
                      (cons 'a752 address) store
                      (church-get-arity (cons 'a753 address)
                        store church-function-signature)))
                   (cons 'a754 address) store
                   (church-get-name (cons 'a755 address)
                     store church-function-signature)))
                (cons 'a756 address) store
                (church-uniform-draw (cons 'a757 address)
                  store church-function-signatures))))
           (church-combine-application-parts church-pair)
           (church-gen-args
            (lambda
              (address store church-arity church-vars
                church-function-signatures)
              (church-repeat (cons 'a758 address) store
                church-arity
                (lambda (address store)
                  (church-gen-expr (cons 'a759 address)
                    store church-vars
                    church-function-signatures)))))
           (church-gen-var church-uniform-draw))
    (begin
      (church-pretty-print (cons 'a760 address) store
        (church-gen-bool (cons 'a761 address) store))
      (church-pretty-print (cons 'a762 address) store
        (church-gen-func-signatures (cons 'a763 address)
          store 4))
      (church-pretty-print (cons 'a764 address) store
        (church-gen-program (cons 'a765 address) store)))))
